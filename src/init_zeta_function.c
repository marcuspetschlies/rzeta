...
int init_refrot_list ( int rotref_list[48][2][3], int rotref_selection_permutation[4][4][48][3], int rotref_selection_sign[4][4][48][3], int rotref_number[4][4] ) {
  ...
  for( int iclass = 0; iclass<8; iclass++) {
    ...
    int const etype = 2*(int)(vector_types[iclass][0] < vector_types[iclass][1]) + (int)(vector_types[iclass][1] < vector_types[iclass][2]);
    int const ztype = (int)(vector_types[iclass][0] == 0) + (int)(vector_types[iclass][1] == 0) + (int)(vector_types[iclass][2] == 0);

    /* fprintf(stdout, "# [init_refrot_list] vector %3d%3d%3d e-type %d z-type %d\n", vector_types[iclass][0], vector_types[iclass][1], vector_types[iclass][2], etype, ztype); */

    int nrotref = 0;
    for( int k=0; k<48; k++) {
      int const p[3] = { vector_types[iclass][ rotref_list[k][0][0] ] * rotref_list[k][1][0],
                         vector_types[iclass][ rotref_list[k][0][1] ] * rotref_list[k][1][1],
                         vector_types[iclass][ rotref_list[k][0][2] ] * rotref_list[k][1][2] };

      // check against previous rot-refs
      int have_rotref = 0;
      for( int i=nrotref-1; i>=0; i--) {
        int const q[3] = { vector_types[iclass][ rotref_selection_permutation[etype][ztype][i][0] ] * rotref_selection_sign[etype][ztype][i][0],
                           vector_types[iclass][ rotref_selection_permutation[etype][ztype][i][1] ] * rotref_selection_sign[etype][ztype][i][1],
                           vector_types[iclass][ rotref_selection_permutation[etype][ztype][i][2] ] * rotref_selection_sign[etype][ztype][i][2] };
...
/*
  for( int iclass = 0; iclass<8; iclass++) {
    ...
    for( int k=0; k<rotref_number[etype][ztype]; k++) {
